...
def main() -> int:
    ...
    for uid, row_idx in df.groupby("unit_id", sort=False).groups.items():
        ...
        elig = [int(p) for p in idx if int(d_aligned[p]) != b0]
        if not elig:
            for p in idx:
                mob_arr[p] = b0 + 1
            continue
        sc_pairs = sorted(((float(score_dev[p]), p) for p in elig), reverse=True)
        # 约 30% 格点与静态不同：约 1/8 单元取 2 个动态时段…
        n_take = 2 if (zlib.crc32(str(uid).encode("utf-8")) % 4 == 0) else 1
        n_take = min(n_take, len(sc_pairs))
        change_idx = {p for _, p in sc_pairs[:n_take]}
        for p in idx:
            if p in change_idx:
                mob_arr[p] = int(d_aligned[p]) + 1
            else:
                mob_arr[p] = b0 + 1

    df["mob_state"] = [f"R{int(x)}" for x in mob_arr]
    for i in range(7):
        df[f"p_R{i + 1}"] = proba[:, i]
