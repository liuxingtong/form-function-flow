...
def _neighbor_prob_matrix(edges: pd.DataFrame, p_mat: np.ndarray, unit_ids: list[str], uid_index: dict[str, int], t_ids: list[str]) -> np.ndarray:
    ...
    for ti, tid in enumerate(t_ids):
        base = ti * n
        idx_slice = np.arange(n) + base
        sub_p = p_mat[idx_slice]  # (n, K)
...
def _transition_matrices_soft(
    ...
    """
    n = len(unit_ids)
    t_index = {t: i for i, t in enumerate(t_ids)}
    mats: dict[tuple[str, str], np.ndarray] = {}
    for t0, t1 in T_PAIRS:
        ti0, ti1 = t_index[t0], t_index[t1]
        m = np.zeros((k, k), dtype=float)
        for uid in unit_ids:
            i0 = ti0 * n + uid_index[uid]
            i1 = ti1 * n + uid_index[uid]
...
def main() -> None:
    ...
    # 转移矩阵：软概率流（与 merged / p_g 行序一致：T_ORDER × sorted(unit_id)）
    unit_ids = sorted(merged["unit_id"].unique().tolist())
    uid_index = {u: i for i, u in enumerate(unit_ids)}
    trans_mats = _transition_matrices_soft(merged, p_g, unit_ids, uid_index, T_ORDER, k)
