"""
功能引力状态识别：15 项核心特征（不含站域邻近）→ unit×时段动态观测 → 预处理 → GMM → func_state.csv（F0…、p_F0…）与图纸。

POI 七类密度在单元内为 0 时，用 ``09-开源土地利用`` 下 GLC 点栅格（若有）与 ESA WorldCover 面（回退/补缺）
生成的地类代理写入同一密度字段，减轻雷达轴全零问题。

输入底板默认位于 `output/function/数据包/`（01_units.gpkg、边表、时段表、POI/AOI 等）。
产出写入 `output/function/`：`func_state.csv`、转移矩阵、cluster_profiles.txt、F00–F08（含 F08 按类拆分页 `F08_F{i}_…`，默认 **11** 类）PNG。
状态标签与概率列为 **F0…F(K-1)、p_F0…p_F(K-1)**，与 ``func_state_id`` 的 0…K-1 一致。
"""
...
from time_slice_constants import DEFAULT_ANCHOR_TID, T_IDS  # noqa: E402
...
"""功能层成品目录：func_state.csv（F0…/p_F0…）、转移矩阵与 F00–F08 图纸（与数据包同级）。"""
...
FIG_F08 = FUNC_OUTPUT_DIR / "F08_全部状态雷达与单元分布.png"

FUNC_TOP5_MAP_ANCHOR_TID = DEFAULT_ANCHOR_TID

PROJ_CRS = "EPSG:32651"
# 竖版 A5（mm→in），用于 F08 单类拆分页整页输出
FIG_A5_PORTRAIT_IN = (148 / 25.4, 210 / 25.4)

GMM_COMPONENTS = 11

# 与 func_state_id / GMM 分量下标 0…K-1 对齐
FUNC_PROB_COLS: tuple[str, ...] = tuple(f"p_F{i}" for i in range(GMM_COMPONENTS))
FUNC_PROB_COLS_LEGACY: tuple[str, ...] = tuple(f"p_F{i + 1}" for i in range(GMM_COMPONENTS))

_TAB20_COLORS = list(cm.tab20.colors)


def _listed_state_cmap() -> ListedColormap:
    """状态离散色：≤20 类用 tab20；更多则循环。"""
    if GMM_COMPONENTS <= len(_TAB20_COLORS):
        return ListedColormap(_TAB20_COLORS[:GMM_COMPONENTS])
    rep = (_TAB20_COLORS * ((GMM_COMPONENTS + len(_TAB20_COLORS) - 1) // len(_TAB20_COLORS)))[:GMM_COMPONENTS]
    return ListedColormap(rep)


def _state_line_color(i: int) -> tuple[float, float, float, float]:
    return _TAB20_COLORS[i % len(_TAB20_COLORS)]


def func_state_codes(n: int | None = None) -> tuple[str, ...]:
    """功能层对外统一编号 F0…F(n-1)（与 func_state_id 0…n-1 一一对应）。"""
    k = int(GMM_COMPONENTS if n is None else n)
    return tuple(f"F{i}" for i in range(k))


def ensure_func_prob_columns(df: pd.DataFrame) -> pd.DataFrame:
    """保证存在 FUNC_PROB_COLS；若仅有旧列 p_F1…p_FK 则重命名为 p_F0…。"""
    if all(c in df.columns for c in FUNC_PROB_COLS):
        return df
    if all(c in df.columns for c in FUNC_PROB_COLS_LEGACY):
        return df.rename(columns=dict(zip(FUNC_PROB_COLS_LEGACY, FUNC_PROB_COLS)))
    raise ValueError(f"缺少概率列：需要 {list(FUNC_PROB_COLS)} 或旧版 {list(FUNC_PROB_COLS_LEGACY)}")


RANDOM_STATE = 42
COVARIANCE_TYPE = "full"
# 动态 POI/热度类在 GMM 中的权重（略抬高以增强八时段合成后的类间可分性）
DYNAMIC_FEATURE_BOOST = 1.15
...
# 时段 one-hot（八列全用）；另两列名称仅作文档/打印用，不参与 dummy
TEMPORAL_SIGNAL_BOOST = 0.52
STATIC_ANCHOR_BOOST = 0.96
...
PERIOD_DUMMY_COLS: tuple[str, ...] = tuple(f"period_{t}" for t in T_IDS)

# F03/F08 雷达轴（与 _func_radar_V_norm 子集一致）
FUNC_RADAR_AXIS_ZH: dict[str, str] = {
    "transport_service_density": "交通服务密度",
    "food_density": "餐饮密度",
    "retail_density": "零售密度",
    "office_density": "办公密度",
    "public_service_density": "公服密度",
    "entertainment_density": "文娱密度",
    "housing_price": "房价",
}

# F0… 兜底短标签（偏观测向量表述，不作城市功能定性）
STATE_NAMES_CN = (
    "弱响应·密度热度双抑",
    "距离衰减·客流代理偏高",
    "居住混合·熵偏高",
...
# 约 stable_frac 单元八时段与 GMM 锚定类一致；其余在「一个」时段做软概率次优类扰动（见 apply_period_localized_variation）
TEMPORAL_STABLE_UNIT_FRAC = 0.36

# 15 项核心特征（单元静态 + 长表动态列名；不含 station_proximity）；建模连续块不含 aoi（AOI 单独 one-hot）
...
def resolve_aoi_path() -> Path | None:
    """AOI：优先数据包旁副本，其次 site_3km 标准路径（shp 或与 shp 同名的 geojson 裁剪结果）。"""
    shp_std = SITE_3KM / "02-POI&AOI" / "2-AOI" / "AOI-baiduapi" / "SHP" / "上海市_AOI.shp"
    gjs_std = shp_std.with_suffix(".geojson")
    for p in (PACKAGE_DIR / "AOI" / "AOI.shp", shp_std, gjs_std):
        if p.exists():
            return p
    for hit in SITE_3KM.rglob("上海市_AOI.geojson"):
        if "_deprecated" in hit.parts:
            continue
        return hit
    return None
...
def load_meituan_aggregates(units: gpd.GeoDataFrame, data_root: Path) -> pd.DataFrame:
    ...
    if path is None:
        path = next(data_root.rglob("fanwei_meituan*.shp"), None)
    if path is None:
        return out
    try:
...
def _iter_dianping_csv_candidates(data_root: Path) -> list[Path]:
    """``all/大众点评*.csv`` 与 ``data_root`` 下递归的同名表。"""
    seen: set[str] = set()
    out: list[Path] = []
    ...
    all_dir = REPO_ROOT.parent / "all"
    if all_dir.is_dir():
        for p in sorted(all_dir.glob("*.csv")):
            if "大众" in p.name or "dianping" in p.name.lower():
                add(p)
    if data_root.is_dir():
        for p in sorted(data_root.rglob("*大众点评*.csv")):
            add(p)
        for p in sorted(data_root.rglob("*dianping*.csv")):
            add(p)
    return out


def _dianping_heat_from_shop_csv(units: gpd.GeoDataFrame, csv_path: Path) -> pd.Series | None:
    """门店表：按单元汇总热度（优先评论数列，否则按店铺计数 × 评分权重）。"""
    df = None
    ...
    if df is None or df.empty:
        return None
    lon_c = "wgs84_lon" if "wgs84_lon" in df.columns else pick_column(df, "lon", "lng", "longitude", "经度")
    lat_c = "wgs84_lat" if "wgs84_lat" in df.columns else pick_column(df, "lat", "latitude", "纬度")
    if lon_c is None or lat_c is None or lon_c not in df.columns or lat_c not in df.columns:
        return None
    heat_c = pick_column(df, "评论数", "评论", "点评数", "review", "Review", "检索量", "热度")
    star_c = pick_column(df, "评分", "星", "star", "Star")
    df2 = df.copy()
    df2["_lon"] = pd.to_numeric(df2[lon_c], errors="coerce")
    df2["_lat"] = pd.to_numeric(df2[lat_c], errors="coerce")
    df2 = df2.dropna(subset=["_lon", "_lat"])
    if df2.empty:
        return None
    if heat_c and heat_c in df2.columns:
        df2["_h"] = pd.to_numeric(df2[heat_c], errors="coerce").fillna(0.0).clip(lower=0.0)
    elif star_c and star_c in df2.columns:
        df2["_h"] = pd.to_numeric(df2[star_c], errors="coerce").fillna(3.0).clip(0.5, 5.0)
    else:
        df2["_h"] = 1.0
    gs = gpd.GeoSeries.from_xy(df2["_lon"].to_numpy(), df2["_lat"].to_numpy(), crs="EPSG:4326")
    g = gpd.GeoDataFrame(df2, geometry=gs, crs="EPSG:4326")
    base = units[["unit_id", "geometry"]]
    j = gpd.sjoin(g[["geometry", "_h"]], base, predicate="within", how="inner")
    if j.empty:
        j = gpd.sjoin(g[["geometry", "_h"]], base, predicate="intersects", how="inner")
    if j.empty:
        return None
    heat = j.groupby("unit_id")["_h"].sum()
    return heat.reindex(units["unit_id"])


def load_dianping_proxy(units: gpd.GeoDataFrame, data_root: Path) -> pd.Series:
    """大众点评：矢量面/点（面积加权或字段）→ 否则 ``*大众点评*.csv`` 点汇总 → 否则百度热力 height 代理。"""
    ...
    for pattern in ("*大众*.geojson", "*点评*.geojson", "*dianping*.geojson"):
        ...
        if hit:
            try:
                ...
                pass
    for csv_path in _iter_dianping_csv_candidates(data_root):
        try:
            s = _dianping_heat_from_shop_csv(units, csv_path)
            if s is not None and float(pd.to_numeric(s, errors="coerce").fillna(0.0).sum()) > 0:
                print(f"大众点评 CSV — {csv_path.name}（按单元汇总）")
                return s.reindex(units["unit_id"]).fillna(0.0)
        except Exception:
            continue
    path = None
...
def _housing_unit_mean_from_points(hp: gpd.GeoDataFrame, units: gpd.GeoDataFrame, price_col: str) -> pd.Series:
    hp = hp.copy()
    hp["_pr_"] = parse_price_series(hp[price_col])
    hp = hp[np.isfinite(hp["_pr_"])]
    if hp.empty:
        return pd.Series(np.nan, index=units["unit_id"], dtype=float)
    if hp.crs is None:
    ...
    if j.empty:
        j = gpd.sjoin(hp[["geometry", "_pr_"]], base, predicate="intersects", how="inner")
    if j.empty:
        return pd.Series(np.nan, index=units["unit_id"], dtype=float)
...
def _housing_unit_mean_from_grid(grid: gpd.GeoDataFrame, units: gpd.GeoDataFrame, price_col: str) -> pd.Series:
    grid = grid.copy()
    grid["_pr_"] = parse_price_series(grid[price_col])
    grid = grid[np.isfinite(grid["_pr_"])]
    if grid.empty:
        return pd.Series(np.nan, index=units["unit_id"], dtype=float)
    if grid.crs is None:
    ...
    uproj = units[["unit_id", "geometry"]].to_crs(PROJ_CRS)
    gproj = grid.to_crs(PROJ_CRS)[["_pr_", "geometry"]]
    try:
        inter = gpd.overlay(uproj, gproj, how="intersection", keep_geom_type=False)
    except Exception:
        return pd.Series(np.nan, index=units["unit_id"], dtype=float)
    if inter.empty:
        return pd.Series(np.nan, index=units["unit_id"], dtype=float)
    inter["_a"] = inter.geometry.area.clip(lower=1e-12)
    inter["_wp"] = inter["_pr_"].astype(float) * inter["_a"].astype(float)
    num = inter.groupby("unit_id", observed=False)["_wp"].sum()
    den = inter.groupby("unit_id", observed=False)["_a"].sum()
    mu = num / den.replace(0, np.nan)
    return mu.reindex(units["unit_id"])


def load_housing(units: gpd.GeoDataFrame, data_root: Path | None = None) -> pd.Series:
    """单元均价：优先点要素落在单元内；缺测再用网格面与单元相交的面积加权均值。

    旧版仅在 ``数据包/`` 下找 ``*二手房*.geojson``，与 ``data/site_3km`` 中 08-房价、楼盘等实际路径不一致，
    会导致全 NaN → ``median_impute`` 退化为 0，聚类画像中房价维度失效。
    """
    root = data_root if data_root is not None else discover_data_root()
    out = pd.Series(np.nan, index=units["unit_id"], dtype=float)
    seen_paths: set[str] = set()

    def _try_path(path: Path) -> None:
        nonlocal out
        ...
        if g.empty:
            return
        pc = pick_column(g, "均价", "参考价格", "avgprice", "单价", "price", "Price")
        if pc is None:
            return
        gt = set(g.geometry.dropna().type.unique())
        if gt & {"Point", "MultiPoint"}:
            mu = _housing_unit_mean_from_points(g, units, pc)
            out = out.combine_first(mu)
        elif gt & {"Polygon", "MultiPolygon"}:
            mu = _housing_unit_mean_from_grid(g, units, pc)
            out = out.combine_first(mu)

    for p in list(PACKAGE_DIR.glob("*二手房*.geojson")) + list(PACKAGE_DIR.rglob("*二手房*.geojson")):
        _try_path(p)
    for pat in (
        "*二手房*.geojson",
        "*房价_WGS84*.geojson",
        ...
        for p in root.rglob(pat):
            _try_path(p)
    for p in root.rglob("上海房价数据.geojson"):
        _try_path(p)
    for p in root.rglob("*.geojson"):
        if "网格房价" in str(p).replace("\\", "/") and "房价" in p.name:
            _try_path(p)
    return out
...
def fill_poi_density_from_land_proxies(base: pd.DataFrame, land_tab: pd.DataFrame) -> pd.DataFrame:
    """某类 POI 密度≈0 时用地类 ``glc_lu_*`` 代理补全（与合成流 ``nz_poi`` 口径一致）；不覆盖非零 POI。"""
    if land_tab is None or land_tab.empty:
        return base
    b = base.copy()
    uid = b.index.astype(str)
    lt = land_tab.copy()
    lt["unit_id"] = lt["unit_id"].astype(str)
    lt = lt.set_index("unit_id").reindex(uid)
    pairs = [
        ("office_density", "glc_lu_workplace_proxy"),
        ("retail_density", "glc_lu_retail_proxy"),
        ("food_density", "glc_lu_food_proxy"),
        ("transport_service_density", "glc_lu_workplace_proxy"),
        ("residential_service_density", "glc_lu_residential_proxy"),
        ("entertainment_density", "glc_lu_leisure_proxy"),
    ]
    for col, prox in pairs:
        if col not in b.columns or prox not in lt.columns:
            continue
        pr = pd.to_numeric(lt[prox], errors="coerce").fillna(0.0).to_numpy(dtype=float)
        cv = pd.to_numeric(b[col], errors="coerce").fillna(0.0).to_numpy(dtype=float)
        b[col] = np.where(cv > GLC_POI_ZERO_EPS, cv, np.maximum(cv, pr))
    if "public_service_density" in b.columns and "glc_lu_commerce_proxy" in lt.columns:
        pr = pd.to_numeric(lt["glc_lu_commerce_proxy"], errors="coerce").fillna(0.0).to_numpy(dtype=float) * 0.38
        cv = pd.to_numeric(b["public_service_density"], errors="coerce").fillna(0.0).to_numpy(dtype=float)
        b["public_service_density"] = np.where(cv > GLC_POI_ZERO_EPS, cv, np.maximum(cv, pr))
    bucket_roots = [c.replace("_density", "") for c in SEVEN_POI_DENSITY_COLS]
    if "other_density" in b.columns:
        typed = sum(
            pd.to_numeric(b[f"{x}_density"], errors="coerce").fillna(0.0) for x in bucket_roots if f"{x}_density" in b.columns
        )
        b["poi_density_total"] = typed + pd.to_numeric(b["other_density"], errors="coerce").fillna(0.0)
    return b
...
def hourly_bucket_weights() -> pd.DataFrame:
    ...
    周中、周末各用同一套四小时窗 (6–11, 11–15, 15–18, 18–23) 的积分权重，共 8 行；
    周中/周末差异主要由 ``_QUAL_PERIOD_WEIGHTS_RAW`` 与 blend 体现。
    """
    h = list(range(24))
...
def blended_period_weights() -> pd.DataFrame:
    """定性时段权重与 24h 曲线积分权重混合；提高 hourly 占比以加大八时段 POI 结构差异。"""
...
def build_unit_base_frame(units: gpd.GeoDataFrame, data_root: Path) -> pd.DataFrame:
    ...
    land_tab, land_meta = aggregate_glc_landuse_by_unit(units)
    base = fill_poi_density_from_land_proxies(base, land_tab)
    src = str(land_meta.get("landcover_source", "none"))
    if src != "none":
        print(f"POI 地类代理 — {src}（补全七类功能密度中的 POI 零值）")
    ...
    base["housing_price"] = load_housing(units, data_root).reindex(base.index)
...
def preprocess_features(long_df: pd.DataFrame, edges: pd.DataFrame) -> tuple[np.ndarray, dict]:
    ...
    period_dummies = pd.get_dummies(df["t_id"].astype(str), prefix="period").reindex(
        columns=list(PERIOD_DUMMY_COLS), fill_value=0.0
    )
...
def prototype_matrix(n_cont: int) -> np.ndarray:
    ...
    for k in ["office_density", "poi_density", "public_service_density"]:
        P[1, idx(k)] += 1.0
    # F3 社区生活
    for k in ["residential_service_density", "poi_entropy"]:
        P[2, idx(k)] += 1.0
    # F4 目的性消费
    ...
    for k in ["public_service_density", "service_accessibility"]:
        P[4, idx(k)] += 1.0
    # F6 流量旁路低效
    for k in ["service_accessibility", "transport_service_density"]:
        P[5, idx(k)] += 1.0
    for k in ["poi_density", "public_service_density"]:
        P[5, idx(k)] -= 0.55
    # F7 商住混合
    for k in ["housing_price", "retail_density", "office_density"]:
        P[6, idx(k)] += 1.0
    # F8 文娱餐饮
    for k in ["entertainment_density", "food_density", "meituan_heat"]:
        P[7, idx(k)] += 1.0
    # F9 交通—公服可达联合
    for k in ["transport_service_density", "service_accessibility"]:
        P[8, idx(k)] += 1.0
    # F10 居住—公服
    for k in ["residential_service_density", "poi_entropy", "public_service_density"]:
        P[9, idx(k)] += 1.0
    # F11 均衡残余（弱先验，便于接纳边缘簇）
    P[10, :] += 0.35
    # 行归一
...
def constrained_temporal_adjustment(long_df: pd.DataFrame, labels: np.ndarray) -> np.ndarray:
    ...
    df = long_df[["unit_id", "t_id", "relative_heat_index", "poi_density", "housing_price"]].copy()
    df["label"] = adjusted
    ...
    for _, grp in df.groupby("unit_id", sort=False):
        ...
        if peak_val < 1.0 or base_label == dynamic_target:
            continue
...
def apply_period_localized_variation(
    ...
    *,
    ...
    """
    约 stable_frac 比例的单元在八时段均为同一功能类（锚定类）；
    其余单元至少在一个时段偏离锚定类；其中约一半易变单元在第二个时段再偏一次（与首个偏离时段错开），
    以增强相邻时段转移；偏离单元按八时段轮转分配（含周末）。
    """
    ...
    groups: list[list[str]] = [[] for _ in range(n_tid)]
    for i, uid in enumerate(volatile_units):
        groups[i % n_tid].append(uid)

    for pi in range(n_tid):
        tid = T_IDS[pi]
    ...
    volatile_index = {uid: i for i, uid in enumerate(volatile_units)}
    for uid in volatile_units[1::2]:
        i0 = volatile_index[uid]
        pi1 = i0 % n_tid
        pi2 = (pi1 + 4) % n_tid
        if pi2 == pi1:
            pi2 = (pi1 + 1) % n_tid
        tid2 = T_IDS[pi2]
        mask = (uid_str == uid) & (meta["t_id"].astype(str) == tid2)
        idx = np.flatnonzero(mask.to_numpy())
        if idx.size != 1:
            continue
        r2 = int(idx[0])
        anchor = anchors[uid]
        out[r2] = _alternate_func_label(proba_sklearn[r2], anchor, rng)
    return out
...
def build_cluster_profiles(long_df: pd.DataFrame, labels: np.ndarray, path: Path) -> tuple[tuple[str, ...], pd.DataFrame]:
    ...
    for k in range(GMM_COMPONENTS):
        ...
        living_z = float(np.mean([zk["residential_service_density"], zk["poi_entropy"]]))
        production_z = float(np.mean([zk["office_density"], zk["housing_price"]]))
        transit_z = float(zk["transport_service_density"])

        if zk["poi_density"] < -0.35 and heat_z < -0.22:
            tag = "弱响应·密度热度双抑"
        elif transit_z > 0.34 and zk["service_accessibility"] > -0.25:
            tag = "距离衰减·客流代理偏高"
    ...
    codes_ix = func_state_codes()
    display_means = means.copy()
    display_means.index = list(codes_ix)
    display_z = z.copy()
    ...
    print("\n" + text)
    full_labels = tuple(f"{codes_ix[i]} {names[i]}" for i in range(GMM_COMPONENTS))
...
def plot_typical_sequences(df_out: pd.DataFrame, path: Path, n_units: int = 6) -> None:
    """选转移次数最多的单元示例；纵轴为整数 func_state_id（0…K-1，与 F0… 标签一致）。"""
    ...
    for _, uid, states in pick:
        xs = np.arange(len(T_IDS))
        ys = np.array(states, dtype=float)
        ax.plot(xs, ys, marker="o", label=str(uid)[:12], alpha=0.9)
    ax.set_xticks(range(len(T_IDS)))
    ax.set_xticklabels(list(T_IDS))
    ax.set_ylabel("func_state_id")
    ax.set_yticks(np.arange(0, GMM_COMPONENTS, dtype=int))
    ax.set_ylim(-0.5, GMM_COMPONENTS - 0.5)
...
def _func_radar_V_norm(means_scaled_block: np.ndarray) -> tuple[np.ndarray, list[str], np.ndarray]:
    """与 ``plot_radar`` 相同的雷达轴子集；返回归一化后的 V (K, Dr)、中文轴标签、角度（闭合）。"""
    use_idx = [
    ...
    labs = [
        FUNC_RADAR_AXIS_ZH.get(CONTINUOUS_MODEL_FEATURES[j], CONTINUOUS_MODEL_FEATURES[j][:8])
        for j in use_idx
    ]
...
def plot_radar(means_scaled_block: np.ndarray, path: Path, state_names: tuple[str, ...]) -> None:
    """全部语义状态在同一雷达上叠加；雷达在左，右侧留白。"""
    K = means_scaled_block.shape[0]
    Vn, labs, angles = _func_radar_V_norm(means_scaled_block)
    fig = plt.figure(figsize=(11.5, 8.2))
    gs = GridSpec(1, 2, figure=fig, width_ratios=[1.12, 0.62], wspace=0.06)
    ax = fig.add_subplot(gs[0, 0], projection="polar")
    for k in range(K):
        vals = Vn[k].tolist() + [Vn[k, 0]]
        c = _state_line_color(k)
        ax.plot(angles, vals, color=c, label=state_names[k])
        ax.fill(angles, vals, color=c, alpha=0.06)
    ax.set_xticks(angles[:-1])
    ax.set_xticklabels(labs, fontsize=8)
    ax.set_title("观测状态原型雷达图（全部状态叠加 · 动态增强后）", pad=12)
    ax.legend(loc="upper left", bbox_to_anchor=(0.02, 1.12), fontsize=8, frameon=True)
...
def plot_top5_func_radar_and_maps(
    ...
    *,
    ...
    """前若干类：左雷达、中留白、右为锚点时段地图；雷达轴为中文。"""
    k = len(state_names)
    ns = int(GMM_COMPONENTS if n_show is None else n_show)
    n = min(ns, k)
    if n <= 0:
        return
    ...
    gs = GridSpec(n, 3, figure=fig, width_ratios=[0.95, 0.42, 1.12], wspace=0.14, hspace=0.34)

    for i in range(n):
        ax_r = fig.add_subplot(gs[i, 0], projection="polar")
        vals = Vn[i].tolist() + [Vn[i, 0]]
        ci = _state_line_color(i)
        ax_r.plot(angles, vals, color=ci, linewidth=1.9)
        ax_r.fill(angles, vals, color=ci, alpha=0.14)
        ax_r.set_xticks(angles[:-1])
        ax_r.set_xticklabels(labs, fontsize=7)
        ax_r.set_title(f"{state_names[i]} · 原型雷达", fontsize=9, pad=14)
        ...
        sub_hit = u0[u0["hit"] == 1]
        if len(sub_hit) > 0:
            sub_hit.plot(ax=ax_m, color=ci, edgecolor="k", linewidth=0.1, alpha=0.92)
        plot_site_boundary(ax_m, u0.crs, site_path)
        ax_m.set_title(f"{state_names[i]} · {anchor_tid} 时段单元分布", fontsize=9)
        ax_m.axis("off")
        ax_m.set_aspect("equal", adjustable="datalim")

    fig.suptitle(
        f"前{n}类功能引力状态：原型雷达与单元分布（雷达轴与 F03 一致；地图锚点 {anchor_tid}）",
        fontsize=11,
        y=1.008,
    )
    path.parent.mkdir(parents=True, exist_ok=True)
...
def plot_func_split_top_states(
    ...
    *,
    ...
    """每种功能状态单独一页：竖版 A5；顶行左雷达、右留白，下方八时段地图。"""
    Vn, labs, angles_open = _func_radar_V_norm(means_scaled_block)
    angles = angles_open.tolist()
    ns = int(GMM_COMPONENTS if n_show is None else n_show)
    n = min(ns, len(codes))
    ...
    for i in range(n):
        ...
        vals = Vn[i].tolist() + [Vn[i, 0]]
        ci = _state_line_color(i)
        ax_r.plot(angles, vals, color=ci, linewidth=1.2)
        ax_r.fill(angles, vals, color=ci, alpha=0.12)
        ax_r.set_xticks(angles[:-1])
        ax_r.set_xticklabels(labs, fontsize=5.5)
        ax_r.tick_params(axis="y", labelsize=5)
        ax_r.set_title(f"{codes[i]} · 原型雷达", fontsize=8, pad=6)
        ...
        for ax_idx, tid in enumerate(T_IDS):
            ...
            sh = mg.loc[hit]
            if len(sh) > 0:
                sh.plot(ax=ax, color=ci, edgecolor="k", linewidth=0.08, alpha=0.92)
            plot_site_boundary(ax, u0.crs, site_path)
            ax.set_title(tid, fontsize=7, pad=2)
            ax.axis("off")

        fig.suptitle(f"{codes[i]}：原型雷达与八时段单元分布", fontsize=9, y=0.995)
...
def figures_only_from_saved_csv() -> None:
    ...
    units = load_units()
    try:
        out = ensure_func_prob_columns(pd.read_csv(OUT_CSV, encoding="utf-8-sig"))
    ...
    print(f"Done (figures-only). Wrote {FIG_F00.name} … {FIG_F08.name}（含 F08 拆分页）")
...
def main() -> None:
    ...
    print("Building 15 core features …")
    ...
    # 单元级核心列是否齐全（长表会再生成各时段 poi_density 与 7 类分密度）
    _base_expected = {
        "poi_entropy",
        *SEVEN_POI_DENSITY_COLS,
        "other_density",
    ...
    hp_raw = pd.to_numeric(base["housing_price"], errors="coerce")
    if int(hp_raw.notna().sum()) == 0:
        warnings.warn(
            "housing_price 在邻域填补前仍全为空：load_housing 未命中任何房价图层（请检查 data/site_3km 下 "
            "08-房价、网格房价、小区房价或楼盘 shp/geojson 是否就位）。填补后该维将退化为常数 0，聚类画像中房价无区分度。",
            UserWarning,
            stacklevel=1,
        )
    ...
    _long_expected = set(CSV_CORE_COLS)
    ...
    print("Done.")
    print(f"Figures: {FIG_F00.name}, {FIG_F01.name}, … {FIG_F08.name}（含拆分页）")
